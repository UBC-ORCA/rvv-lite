...
object GenCoreRvvLite{
  ...
  def main(args: Array[String]) {
    ...
    // sbt compile "run-main vexriscv.GenCoreRvvLite -d --iCacheSize=1024"
    val parser = new scopt.OptionParser[ArgConfig]("VexRiscvGen") {
      ...
      opt[Boolean]("rvvLite")   action { (v, c) => c.copy(rvvLite = v)   } text("If true, add bus for rvvLite module")
